...
object KonaneLogic {
    ...
    def setupBoard(board: Board, rows: Int, cols: Int, rand: MyRandom): (Board, List[Coord2D], MyRandom) = {
        ...
        val adjacents = List((rb + 1, cb), (rb - 1, cb), (rb, cb + 1), (rb, cb - 1))
            .filter(c => board.get(c).contains(Stone.White))
    ...
    def randomMove(lstOpenCoords: List[Coord2D], rand: MyRandom): (Coord2D, MyRandom) = {
        ...
        (lstOpenCoords(randomIndex), nextRand)
    }
    ...
    def applyMoveSequence(board: Board, player: Stone, startFrom: Coord2D, destinations: List[Coord2D],
                          rows: Int, cols: Int, openCoords: List[Coord2D]): Option[(Board, List[Coord2D])] = {
        @tailrec
        def loop(currBoard: Board, currOpen: List[Coord2D], from: Coord2D, remaining: List[Coord2D]): Option[(Board, List[Coord2D])] =
            remaining match {
    ...
    def continueCapturePure(board: Board, player: Stone, lastPos: Coord2D, choiceOpt: Option[Coord2D],
                            rows: Int, cols: Int, openCoords: List[Coord2D]): Option[(Board, List[Coord2D], Boolean)] =
        choiceOpt match {
            ...
            case Some(dest) =>
                ...
                if (!possible.contains(dest)) None
                    ...
                    Some((b2, o2, canMoveAgain(b2, player, dest, rows, cols)))
                }
        }
    ...
    def getAllValidMoves(board: Board, player: Stone, rows: Int, cols: Int): List[(Coord2D, Coord2D)] = {
        ...
        def loop(remaining: List[Coord2D], acc: List[(Coord2D, Coord2D)]): List[(Coord2D, Coord2D)] = remaining match {
            ...
            case from :: tail =>
                val pairs = getValidMovesForPiece(board, player, from, rows, cols).map(to => (from, to))
                loop(tail, pairs.reverse ++ acc)
        }
        loop(board.keys.toList, Nil)
    }
    ...
    def getDestinations(mvs: List[(Coord2D, Coord2D)]): List[Coord2D] = {
        ...
        def loop(remaining: List[(Coord2D, Coord2D)], acc: List[Coord2D]): List[Coord2D] = remaining match {
            case Nil           => acc.reverse
            case (_, to) :: tail => loop(tail, to :: acc)
        }
        loop(mvs, Nil)
    }
    ...
    def playRandomly(board: Board, r: MyRandom, player: Stone,
        ...
        moves match {
            ...
            case _ =>
                val (chosenTo, nextRand) = f(getDestinations(moves), r)
    ...
    def boardToString(board: Board, rows: Int, cols: Int): String = {
        ...
        val boardBody = (0 until rows).foldLeft("") { (acc, r) =>
        ...
        if (trimmed.length < 2) None
            ...
            scala.util.Try(rowPart.toInt) match {
                case scala.util.Success(row) if colChar >= 'A' && colChar <= 'Z' =>
                    Some((row, colChar.toInt - 'A'.toInt))
                case _ => None
            }
        }
    }
    ...
    def coordToString(coord: Coord2D): String =
        (coord._2 + 'A'.toInt).toChar.toString + coord._1.toString

    def isValidDimension(r: Int, c: Int): Boolean =
        r > 2 && r <= 20 && c > 2 && c <= 20

    def getWinner(board: Board, currentPlayer: Stone, rows: Int, cols: Int): Option[Stone] =
        getAllValidMoves(board, currentPlayer, rows, cols) match {
            case Nil => Some(if (currentPlayer == Stone.Black) Stone.White else Stone.Black)
            case _   => None
        }
}